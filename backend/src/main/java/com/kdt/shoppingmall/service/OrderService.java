...
public class OrderService {
    ...
    public List<OrderResponse> getOrders(Long memberId) {
        return orderRepository.findByMemberIdOrderByCreatedAtDesc(memberId).stream().map(OrderResponse::from).toList();
    }
    ...
    public PaymentResponse pay(Long memberId, Long orderId) {
        ...
        Payment payment = paymentRepository.save(
                new Payment(order, success ? PaymentStatus.SUCCESS : PaymentStatus.FAILED, order.getTotalPrice()));
    ...
    private List<CartItem> resolveCartItems(Long memberId, OrderCreateRequest request) {
        ...
        return request.cartItemIds().stream()
                .map(id -> cartItemRepository.findById(id)
                        .orElseThrow(() -> new ResourceNotFoundException("장바구니 항목을 찾을 수 없습니다. id=" + id)))
                .peek(cartItem -> {
