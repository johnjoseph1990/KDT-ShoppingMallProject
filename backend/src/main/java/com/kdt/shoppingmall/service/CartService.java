...
public class CartService {
    ...
    public CartService(
            CartItemRepository cartItemRepository, MemberRepository memberRepository, ProductRepository productRepository) {
    ...
    public CartItemResponse addItem(Long memberId, CartItemRequest request) {
        ...
        CartItem cartItem = cartItemRepository.findByMemberAndProduct(member, product)
                ...
                .orElseGet(() -> cartItemRepository.save(new CartItem(member, product, request.quantity())));

        return CartItemResponse.from(cartItem);
    }

    public List<CartItemResponse> getCart(Long memberId) {
        return cartItemRepository.findByMemberId(memberId).stream().map(CartItemResponse::from).toList();
    }
