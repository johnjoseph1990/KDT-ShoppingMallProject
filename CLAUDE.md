...
**브랜치+PR 전략:**
- **사소한 수정·버그픽스** → master 직접 커밋 유지 (멀티PC 규칙과 충돌 없이 단순하게)
- **여러 커밋이 묶이는 대형 기능** → feature 브랜치 + PR 사용 권장
  (실무 워크플로 학습 목적; 리뷰어 없어도 PR 단위로 변경 이력이 명확해짐)
...
**E2E 스모크 (프론트):** 핵심 사용자 경로 5개(홈·목록→상세·회원가입·장바구니·관리자)는
`frontend/e2e/`의 Playwright 스모크로 검증한다. 단위 테스트만으로는 못 잡는 통합·화면·인가
결함을 배포 전에 거른다. 실행법·경계는 `frontend/e2e/README.md` 참고.

**로컬 E2E 선실행 규칙 (필수):** CI E2E가 자동화되어 있지만, 셀렉터·경로·화면 레이아웃을
바꿨을 때는 `git push` 전에 로컬에서 먼저 돌린다. 환경이 이미 떠 있으면 추가 비용 없이
CI 재시도를 예방할 수 있다.

```
화면·셀렉터·경로 수정 → npm run e2e 통과 확인 → git push
백엔드 로직만 수정   → 생략 가능
```
